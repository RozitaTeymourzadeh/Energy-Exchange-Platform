...
var SBC bcdata.SyncBlockChain
var Peers bcdata.PeerList
var Heart bcdata.HeartBeatData
var ifStarted bool

var TA_SERVER = "http://10.7.52.166:6686"
var REGISTER_SERVER = TA_SERVER + "/peer"
var ASK_PEERS = "/block"
var SELF_ADDR = "10.7.52.166:6686" //"localhost:6686"
var EDGEX_IP = "localhost"
var SELF_ID = 0
var FIRST_PEER = "192.168.11.20:6686" //
var BC_DOWNLOAD_SERVER = FIRST_PEER + "/upload"
var RECEIVE_PATH = "/heartbeat/receive"
var STOP_GEN_BLOCK = false
var NONCE_ZERO = "000"

var newTransactionObject bcdata.Transaction
var TxPool bcdata.TransactionPool
var Balance int = 10000
var TransactionMap map[string]bcdata.Transaction
...
func init() {
	...
	TxPool = bcdata.NewTransactionPool()
	SBC = bcdata.NewBlockChain()
	Peers = bcdata.NewPeerList(Peers.GetSelfId(), 32)
	ifStarted = false
	TransactionMap = make(map[string]bcdata.Transaction)
	...
	if len(os.Args) > 1 {
		...
		SELF_ADDR = data.SystemIp() + ":" + os.Args[1]
...
func Start(w http.ResponseWriter, r *http.Request) {
	...
	/* Start Trying Nonce */
	minerKey = bcdata.GenerateKeyPair(4096)
...
func Register() {
	...
	id := int32(result)
	Peers = bcdata.NewPeerList(id, 32)
}
...
func Download() {
	...
	registerData := bcdata.NewRegisterData(Peers.GetSelfId(), peerMapStringValue)
...
func Upload(w http.ResponseWriter, r *http.Request) {
	...
	registerData := bcdata.NewRegisterData(0, "")
...
func HeartBeatReceive(w http.ResponseWriter, r *http.Request) {
	...
	transaction := bcdata.Transaction{}
...
*
 */
func ForwardHeartBeat(heartBeatData bcdata.HeartBeatData) {
...
func StartHeartBeat() {
	...
	for key, value := range PeerMap {
		...
		mpt := p4.MerklePatriciaTrie{}
		heartBearData := bcdata.PrepareHeartBeatData(&SBC, Peers.GetSelfId(), peerMapToJson, SELF_ADDR, false, "", mpt, &minerKey.PublicKey, false, "")
	...
	GetLatestBlock:
		...
		if transaction != nil {
			...
			if Verified && enoughBalance {
				...
				decryptedPKCS1v15 := bcdata.DecryptPKCS(minerKey, encryptedPKCS1v15)
				Verified, _ := bcdata.VerifyPKCS(&minerKey.PublicKey, h, hashed, Signature)
			...
			if strings.HasPrefix(hex.EncodeToString(sum[:]), NONCE_ZERO) {
				...
				transactionJSON, _ = transaction.EncodeToJson()
				heartBeatData := bcdata.PrepareHeartBeatData(&SBC, Peers.GetSelfId(), peerMapJson, SELF_ADDR, true, validateNonce,
...
func Event(w http.ResponseWriter, r *http.Request) {
	...
	case "POST":
		...
		if Balance-PowerFee >= 0 {
			...
			newTransactionObject := bcdata.NewTransaction(eventId, SupplierName, SupplierId, SupplierAddress, ConsumerName, ConsumerId, ConsumerAddress, PowerUnits, Timestamp, PowerFee, Balance, EventType)
			...
			if transactionReady {
				encryptedPKCS1v15 := bcdata.EncryptPKCS(&minerKey.PublicKey, transactionJSON)
				fmt.Println("encryptedPKCS1v15 is:", encryptedPKCS1v15)
				encryptedPKCS1v15Str := string(encryptedPKCS1v15)
				h, hashed, signature := bcdata.SignPKCS(encryptedPKCS1v15Str, minerKey) //Private Key
